...
read flag_480
echo "\n"
echo "${UNDERLINE}${BOLD}do you want to download the 360p${NONE}? y/n"
read flag_360
echo "\n"
...
then
	...
	mkdir source_ts
	aws s3 cp --recursive s3://wynk-msp-vod-staging/livestreaming/GAMING/$mspid/$mspid ./source_ts/ --quiet
...
then
	...
	mkdir 1080_ts
	aws s3 cp --recursive s3://wynk-msp-vod-staging/livestreaming/GAMING/$mspid/"$mspid"_1080p ./1080_ts/ --quiet
...
then
	...
	mkdir 720_ts
	aws s3 cp --recursive s3://wynk-msp-vod-staging/livestreaming/GAMING/$mspid/"$mspid"_720p ./720_ts/ --quiet
...
then
	...
	mkdir 480_ts
	aws s3 cp --recursive s3://wynk-msp-vod-staging/livestreaming/GAMING/$mspid/"$mspid"_480p ./480_ts/ --quiet
...
if [ "$flag_360" = "y" ]
then
	echo "<<<<<<<<<<<<<<<<<<<<<<<< 360_ts processing started >>>>>>>>>>>>>>>>>>>>>>>>>>>"
	mkdir 360_ts
	aws s3 cp --recursive s3://wynk-msp-vod-staging/livestreaming/GAMING/$mspid/"$mspid"_360p ./360_ts/ --quiet
	cat ./360_ts/*.ts > ./360_ts/360.ts
	ffmpeg -i ./360_ts/360.ts -acodec copy -vcodec copy 360.mp4 -loglevel quiet
	echo "\n"
fi
...
then
	echo ""
	echo "*^*^*^*^*^*^*^*^*^*^*^*^*^*^*^* ${YELLOW}360_result${NONE} *^*^*^*^*^*^*^*^*^*^*^*^*^*^*^*"
	ffmpeg -i 360.mp4 -hide_banner
fi
